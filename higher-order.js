...
let totalPopulation = populations.reduce((acc,num)=>acc + num);
...
let myStrongest = monstersInYourPocket.filter( (item, index) =>  (monstersInYourPocket[index].CP > 
...
let ordersTotal = orders.map((el, index, arr) => {
...
//  BEST REsult!!!:
//  let bobsTotal = purchases.filter(c=>c.owner == "Bob")//filter to only bob)
//                 .map(c=>c.owner)//map over it
//                 .
